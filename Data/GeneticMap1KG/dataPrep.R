...
res <- lapply(1:23, function(i){
  ...
  genLink <- paste0("http://mathgen.stats.ox.ac.uk/impute/ALL_1000G_phase1interim_jun2011_impute/genetic_map_chr", 
                    CHR, "_combined_b37.txt")
  fread(genLink)[, .(CHR = i, BP = position, RECOMB = round(`COMBINED_rate(cM/Mb)`, 2))
                 ][RECOMB > 0, ]
})
...
saveRDS(GeneticMap1KG, "Data/GeneticMap1KG/GeneticMap1KG.rds")
